...
local scene = {
	...
	setup = function(self)
		...
		local retro_font = GFX.font.new("themes/retro/assets/fonts/gamekid_m")
		GFX.setFont(retro_font)
		currentSong = self.retro_music

		-- set menu background
		menuBackground = GFX.image.new("themes/retro/assets/images/menu_background")
	...
	visualEffect = function()
		...
		for i = 1, 3 do
			...
			_x = _x + 32 -- increment x for next firework	
		end
		...
		function effect:update()
			for i, firework in ipairs(fireworks) do  -- #v is the size of v for lists.
				...
				if firework.exploded then
					firework.animation_frame = firework.animation_frame + 0.5
